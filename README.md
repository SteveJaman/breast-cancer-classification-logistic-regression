...
Features are categorized by their Mean, Standard Error (se), and "Worst" (largest) values.

## Project Pipeline

1. **Data Cleaning:** Handled headers, removed non-predictive IDs, and encoded diagnoses (M=1, B=0).
2. **Preprocessing:** Used `StandardScaler` to normalize feature scales for the Logistic Regression algorithm.
3. **Modeling:** Implemented `LogisticRegression` using Scikit-Learn with a 70/30 train-test split.
4. **Evaluation:** Generated classification reports, confusion matrices, and ROC curves to validate reliability.
